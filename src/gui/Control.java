...
public class Control extends JFrame implements KeyListener {
    ...
    /**
     * Boolean tracking if a robot is being used
     */
    boolean usingRobot = false;
    
    /**
     * String tracking which driver is being used
     */
    String string_driver = "Manual"; 
    ...
    /**
     * Boolean tracking if the robot is reliable
     */
    boolean isreliable = true;
    
    /**
     * String recording the reliability of the sensors on the robot
     * Stored in binary (forward, left, right, backward)
     */
	...
	private void handleCommandLineInput(String[] args) {
	    ...
	    String parameter = "DFS";
	    if (argsList.contains("-g")) {
	    	parameter = argsList.get(argsList.indexOf("-g") + 1);
	    }
	    string_driver = "Manual";
	    if (argsList.contains("-d")) {
	    	string_driver = argsList.get(argsList.indexOf("-d") + 1);
	    	if (!string_driver.equals("Manual")) {
	    		usingRobot = true;
	    	}
	    }
	    if (argsList.contains("-r")){
	    	isreliable = false;
	    	sensorString = argsList.get(argsList.indexOf("-r") + 1);
	    }
	    ...
	    case "Boruvka":
	        ...
	        break;
	    //case "Robot":
	    //	msg = "Command line input detected: generating random maze and creating a robot.";
	    //	usingRobot = true;
	    //	((StateTitle)currentState).setBuilder(Order.Builder.DFS);
