...
public class ReliableRobot implements Robot {
	...
	*/
	...
	public int[] getCurrentPosition() throws Exception {
		...
		int[] coordinates = controller.getCurrentPosition();
		
		//checks coordinates inside maze boundaries
		if (coordinates[0] > controller.getMaze().getWidth() || (coordinates[0] < 0 || coordinates[1] >= controller.getMaze().getHeight() || (coordinates[1] < 0 ))){
			throw new Exception();
		}
	...
	*/
	...
	public void move(int distance){
		if (!hasStopped() && battery>0 && distance > 0) {
			...
			if(controller.getMaze().getFloorplan().hasWall(controller.getCurrentPosition()[0], controller.getCurrentPosition()[1], getCurrentDirection())) {
				hasStopped = true;
				return;
			}
	...
	public void jump() {
		if (!hasStopped() && battery>0) {
			...
			//This is checking if the wall being jumped is a border wall and stopping if it is
				Wallboard borderWallboard = new Wallboard(controller.getCurrentPosition()[0], controller.getCurrentPosition()[1], getCurrentDirection());
				if(controller.getMaze().getFloorplan().isPartOfBorder(borderWallboard)) {
					hasStopped = true;
					return;
				}
			}
	...
	public boolean isAtExit() {
		...
		try {
			if (Arrays.equals(getCurrentPosition(),controller.getMaze().getExitPosition())){
				return true;
			}
		} catch (Exception e) {
	...
	public boolean isInsideRoom() {
		...
		int[] coordinates = controller.getCurrentPosition();
		return controller.getMaze().isInRoom(coordinates[0], coordinates[1]);
	}
