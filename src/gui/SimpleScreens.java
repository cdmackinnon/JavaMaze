...
public class SimpleScreens {
	...
	private void redrawFinish(Graphics gc) {
		...
		gc.setColor(ColorTheme.getColor(MazeColors.TITLE_DEFAULT));
		centerString(gc, "Odometer: " + String.valueOf(odometer), 222);
		gc.setColor(ColorTheme.getColor(MazeColors.TITLE_DEFAULT));
		centerString(gc, "Battery: " + String.valueOf(battery), 200);
	}
	
	int odometer;
	float battery; 
	//this function does not work because it is being called in the wrong instance
	//these variables are initialized in Stategenerating and not StateWinning 
	public void youWon(int odometers, float battery) {
		this.odometer = odometers;
		this.battery = battery;
	}
	
	public void youLost(MazePanel panel, int odometer, float battery) {
		Graphics g = panel.getBufferGraphics() ;
		redrawLoser(g, odometer, battery);
	}
	
	/**
	 * Helper method for redraw final screen
	 * @param Odometer 
	 * @param Value
	 * @param gc graphics is the off-screen image
	 */
	private void redrawLoser(Graphics gc, int odometer, float battery) {
		// produce blue background
		drawBackground(gc);
		// write the title 
		updateFontAndColor(gc, largeBannerFont, MazeColors.TITLE_LARGE);
