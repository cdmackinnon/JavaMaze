...
public class StateGenerating extends DefaultOrder implements State {
     ...
     * @throws Exception 
     */
    public void switchFromGeneratingToPlaying(Maze maze) throws Exception {
	    ...
	    if (control.usingRobot) {
	    	ReliableRobot robot;
	    	if (control.isreliable) {
	    		robot = new ReliableRobot();
	    	}
	    	else {
	    		robot = new UnreliableRobot(control);
	    	}
	    	
	        RobotDriver driver;
	        if (control.string_driver.equals("Wizard")) {
	    	driver = new Wizard();
	        }
	        else if(control.string_driver.equals("Wallfollower")){
	        driver = new WallFollower();
	        }
	        else {
	        	throw new Exception("No Driver for the Robot");
	        }
	    	...
	    	control.handleKeyboardInput(UserInput.TOGGLELOCALMAP, KeyEvent.VK_M);
			control.handleKeyboardInput(UserInput.TOGGLEFULLMAP, KeyEvent.VK_Z);
	    	control.handleKeyboardInput(UserInput.TOGGLESOLUTION, KeyEvent.VK_S);
	    	try {
				driver.drive2Exit();
				System.out.println(robot.odometer);
				System.out.println(robot.getBatteryLevel());
		    	view.youWon(robot.odometer, robot.getBatteryLevel());
			} catch (Exception e) {
				view.youLost(panel, robot.getOdometerReading(), robot.getBatteryLevel());
			}
        }
    }
